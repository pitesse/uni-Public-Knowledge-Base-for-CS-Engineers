...
manhattanDistance :: Num a => Point2d a -> Point2d a -> a
manhattanDistance
  (Point2d x0 y0)
  (Point2d x1 y1) =
    (x1 - x0) + (y1 - y0)
...
data Point2dAlt_type_constructor a = Point2dAlt_data_contructor
  { pointX :: a,
    pointY :: a
  }
...
getJustX :: Point2dAlt_type_constructor a -> a
getJustX Point2dAlt_data_contructor {pointX = x} = x
